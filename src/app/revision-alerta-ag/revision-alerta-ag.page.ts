...
@Component({
  selector: 'app-revision-alerta-ag',
  templateUrl: './revision-alerta-ag.page.html',
  styleUrls: ['./revision-alerta-ag.page.scss'],
})
export class RevisionAlertaAgPage implements OnInit {
  noinitban = 0;
  Edificacion;
  otroFi = false;
  Daños = new Array();
        ...
        if (err2 == null && response2 == true) {  
              //this.loading.HideLoading();        
               this.alert.AlertOneButton('Información', 'Revisión exitosa', 'Ok', () => {
                  this.navCtrl.navigateRoot('/listalerta-ag');
                });
          } else {
            //this.loading.HideLoading();
            this.alert.AlertOneButton('Error', 'Error al subir registro');
